...
#define USESERIAL
#define SLEEPTIME_MILLISECONDS 10000 // define the sleep time of 5 minute(s) WARNING: Each module must have different sleep times

// If value is equal to -42 => measure is not correct(used in data validation and displaying in inside)
...
const int MAX_MEASURE_RETRIES
    = 3; // measures up to 3 incorrect values
...
void readBatteryPercentage(RadioPacket* radioPacket);
...
void loop()
{
    ...
    readBatteryPercentage(&radioPacket);
#endif
...
void measureDistance(float& distance)
{
    ...
    for (int i = 0; i < MEASURES_COUNT; i++) {
        float currentDistance = snowMeter.readRangeSingleMillimeters() / 10.00; // read distance in mm, convert to cm
