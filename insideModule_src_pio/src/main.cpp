...
#define OUTSIDE_MODULES_COUNT 2 // set the count of the active modules
// Radio:
#define CE_PIN 17 // SPI chip enable
#define CS_PIN 5 // SPI chip select pin
uint8_t readAddresses[OUTSIDE_MODULES_COUNT][6] = { "1OUTM", "2OUTM" }; // addresses for outside modules (Index+OUTM)
...
void heightSetup()
{
    ...
    char names[OUTSIDE_MODULES_COUNT][50];
...
const char* buildHTTPSURL()
{
    ...
    strcat(httpsURL, insideMeasurer.getTemperature());
    strcat(httpsURL, "&insideHumidity=");
    strcat(httpsURL, insideMeasurer.getHumidity());

    // Append all the outside modules readings to the url c-string:
    for (int i = 0; i < OUTSIDE_MODULES_COUNT; i++) {
        // Append outsideData to http

        // Convert batteryLevel to string:
        ...
        char outsideData[50];
        sprintf(outsideData, "&%i,outsideTemperature=%s", i + 1, outsideMeasurers[i]->getTemperature());
        strcat(httpsURL, outsideData);
        sprintf(outsideData, "&%i,outsideHumidity=%s", i + 1, outsideMeasurers[i]->getHumidity());
        strcat(httpsURL, outsideData);
        sprintf(outsideData, "&%i,snowDepth=%s", i + 1, outsideMeasurers[i]->getSnowDepth());
        strcat(httpsURL, outsideData);
